...
https://www.bilibili.com/video/BV1Rv41177Af?p=5
...
![image-20220416190536589](Redis.assets/image-20220416190536589.png)

### Redis

#### redis底层原理:单线程加多路复用技术
...
![img](Redis.assets/e2d6106091842a670d8e0e9933c1569f.webp)
...
![image-20220416200623992](Redis.assets/image-20220416200623992.png)
...
![image-20220416202440927](Redis.assets/image-20220416202440927.png)
...
![image-20220418093602214](Redis.assets/image-20220418093602214.png)

![image-20220418093732230](Redis.assets/image-20220418093732230.png)

##### HyperLogLog

>求集合中不重复元素的个数

![image-20220418093933408](Redis.assets/image-20220418093933408.png)

##### Geospaatial

> 用于地理位置

![image-20220418094107207](Redis.assets/image-20220418094107207.png)

#### redis事务和锁机制

![image-20220417161653903](Redis.assets/image-20220417161653903.png)

##### Multi, Exec,Discard

![image-20220417161856176](Redis.assets/image-20220417161856176.png)
...
![image-20220417163520543](Redis.assets/image-20220417163520543.png)

###### 乐观锁 

给事务加一个版本号,每个人都能拿到这个版本号,他们执行对应操作后,快的那个人执行完相应操作后就会将版本号更新,另一个人操作时,会相比较自己获取的版本号与当前的版本是否相同,如果不相同就会重新设置自己获取的版本号.然后继续竞争执行操作

![image-20220418204252274](Redis.assets/image-20220418204252274.png)
...
![image-20220418094423786](Redis.assets/image-20220418094423786.png)
...
https://www.bilibili.com/video/BV1Rv41177Af?p=26
...
![image-20220419100432975](Redis.assets/image-20220419100432975.png)
...
![image-20220419104512045](Redis.assets/image-20220419104512045.png)![image-20220419104610301](Redis.assets/image-20220419104610301.png)![image-20220419104639510](C:\Users\full\AppData\Roaming\Typora\typora-user-images\image-20220419104639510.png)
...
![image-20220419141257577](Redis.assets/image-20220419141257577.png)

#### 设置服务:windows下

![image-20220419124344545](Redis.assets/image-20220419124344545.png)
...
![image-20220419190212275](Redis.assets/image-20220419190212275.png)

#### 主机代理模式

> 使用了较多的服务器,不合理

![image-20220419190331827](Redis.assets/image-20220419190331827.png)

#### 无中心集群方式

> 每一台服务器都可以作为进入集群的入口

![image-20220419190420047](Redis.assets/image-20220419190420047.png)

#### 集群设置

> 参照视频: [集群操作](https://www.bilibili.com/video/BV1Rv41177Af?p=37&spm_id_from=pageDriver)

![image-20220419192435879](Redis.assets/image-20220419192435879.png)
...
![image-20220420141852053](Redis.assets/image-20220420141852053.png)
...
![image-20220420203208085](Redis.assets/image-20220420203208085.png)
...
3. 采用布隆过滤器,底层与bitmap类似
   ...
   ![image-20220420202838109](Redis.assets/image-20220420202838109.png)![image-20220420203001035](Redis.assets/image-20220420203001035.png)
...
![image-20220420203913522](Redis.assets/image-20220420203913522.png)
...
![image-20220420205001832](Redis.assets/image-20220420205001832.png)
...
![image-20220420210725939](Redis.assets/image-20220420210725939.png)
...
![image-20220421103353638](Redis.assets/image-20220421103353638.png)
...
![image-20220421103451902](Redis.assets/image-20220421103451902.png)
